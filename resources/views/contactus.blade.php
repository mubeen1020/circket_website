@extends('default')
@section('content')

<div class="holder point">
		<div class="container">
			<div class="point-table-all border">
				<div class="series-drop">
					<div class="row">
						<div class="col-sm-3">
							<div class="border-heading">
							<h5>CONTACT US</h5>
								</div>
						</div>
						</div>
				</div>
                <center>       
				<img alt="MCL banner original.jpg" src="{{ asset('utilsv2/img/others/eoscl-logo.png') }}" style="height:150px"> &nbsp;
<table border="1" cellpadding="0" cellspacing="0" style="width:593px">
	<tbody>
		<tr>
			<td colspan="4" style="width:593px">
			<h3><strong>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Event Ontario Softball Circket
 EXECUTIVES</strong></h3>
			</td>
		</tr>
		<tr>
			<td style="width:23px"><strong>#</strong></td>
			<td style="width:135px"><strong>NAME</strong></td>
			<td style="width:288px"><strong>EMAIL</strong></td>
			<td style="width:147px"><strong>POSITION</strong></td>
		</tr>
		<tr>
			<td style="width:23px">1</td>
			<td style="width:135px">Meraj mubaahir</td>
			<td style="width:288px">meraj.mubashir@gmail.com</td>
			<td style="width:147px">President</td>
		</tr>
		<tr>
			<td style="width:23px">2</td>
			<td style="width:135px">Haroon shaikh Asad </td>
			<td style="width:288px">Haroonasad83@gmail.com</td>
			<td style="width:147px">Lead umpire</td>
		</tr>
		<tr>
			<td style="width:23px">3</td>
			<td style="width:135px">Eoscl inquiry </td>
			<td style="width:288px">Register@eoscl.com</td>
			<td style="width:147px">Treasurer</td>
		</tr>
	</tbody>
</table>
&nbsp;

<!-- <table border="1" cellpadding="1" cellspacing="1" style="width:500px">
	<caption>Contact DRC at info@EventOntarioSoftballCircket.ca</caption>
	<thead>
		<tr>
			<th scope="col">#</th>
			<th scope="col">NAME</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>1</td>
			<td>NABEEL MIRZA</td>
		</tr>
		<tr>
			<td>2</td>
			<td>ADEEL AHMED</td>
		</tr>
		<tr>
			<td>3</td>
			<td>SAQIB JAVAID</td>
		</tr>
		<tr>
			<td>4</td>
			<td>SIRAJ MOOSA</td>
		</tr>
		<tr>
			<td>5</td>
			<td>RAFAY SHAMS</td>
		</tr>
		<tr>
			<td>6</td>
			<td>ZAHOOR BUTT</td>
		</tr>
	</tbody>
</table> -->
</div>
		</div>
	</div>
</center>

@stop